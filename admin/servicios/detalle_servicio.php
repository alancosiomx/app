...
$ticket = trim($_GET['ticket'] ?? ''); // ✅ TRIM para quitar espacios
...
// ✅ Consulta segura con TRIM en la columna también
...
<div id="modalDetalleServicio" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
  <div class="bg-white rounded-lg shadow-xl w-full max-w-3xl p-6 relative">
    ...
    <h2 class="text-lg font-bold mb-4 text-gray-800">
      🔍 Detalle del Servicio - <?= htmlspecialchars(trim($servicio['ticket'])) ?>
    </h2>
    ...
    <div class="overflow-y-auto max-h-[70vh] border rounded-md">
      <table class="w-full text-sm text-left text-gray-700 divide-y">
        <?php foreach ($servicio as $campo => $valor): ?>
          <tr class="border-b">
            <th class="bg-gray-100 px-4 py-2 font-semibold w-1/3">
              <?= ucwords(str_replace("_", " ", $campo)) ?>
            </th>
            <td class="px-4 py-2 whitespace-pre-line">
              <?= nl2br(htmlspecialchars(trim((string)$valor))) ?>
            </td>
          </tr>
        <?php endforeach; ?>
      </table>
