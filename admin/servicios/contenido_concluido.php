...
<p class="text-sm text-gray-600 mb-6">Bienvenido, <strong><?= htmlspecialchars($_SESSION['usuario_nombre'] ?? 'Administrador') ?></strong></p>

<?php include __DIR__ . '/../includes/tabs_servicios.php'; ?>

<?php if (isset($_SESSION['mensaje'])): ?>
  <div class="bg-green-100 text-green-800 px-4 py-2 rounded mb-4"><?= htmlspecialchars($_SESSION['mensaje']) ?></div>
...
<form method="get" class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
  ...
  <div>
    ...
    <input type="date" id="fecha_inicio" name="fecha_inicio" value="<?= htmlspecialchars($fecha_inicio) ?>" class="mt-1 block w-full rounded border-gray-300 shadow-sm text-sm">
  </div>

  <div>
    <label for="fecha_fin" class="block text-sm font-medium text-gray-700">Fecha fin</label>
    <input type="date" id="fecha_fin" name="fecha_fin" value="<?= htmlspecialchars($fecha_fin) ?>" class="mt-1 block w-full rounded border-gray-300 shadow-sm text-sm">
  </div>
  ...
  <div>
    ...
    <input type="text" id="ticket" name="ticket" placeholder="Buscar..." value="<?= htmlspecialchars($ticket_busqueda) ?>" class="mt-1 block w-full rounded border-gray-300 shadow-sm text-sm">
  </div>
    ...
    <tbody class="divide-y divide-gray-200">
      <?php foreach ($servicios as $s): ?>
        <tr class="hover:bg-gray-50">
          ...
          <td class="px-4 py-2"><?= htmlspecialchars($s['fecha_atencion']) ?></td>
          <td class="px-4 py-2"><?= htmlspecialchars($s['resultado'] ?? '—') ?></td>
          <td class="px-4 py-2"><?= htmlspecialchars($s['comentarios']) ?></td>
